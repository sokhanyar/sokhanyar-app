...
import ir.saltech.myapps.stutter.util.getMimeType
...
class VoiceAnalyzeViewModel : ViewModel() {
    private val expectedExtensions = arrayOf(".mp3", ".ogg", ".aac", ".m4a")
    ...
    fun initialVoiceObserving(): RecursiveFileObserver {
        return RecursiveFileObserver(
            ...
            Log.d(
                "TAG",
                "New File is detected an Eitaa Audio: ${file.name} | Date modified: ${file.lastModified()} | NewFile is dir: ${file.isDirectory} || RESL: ${
                    Environment.getExternalStorageState(file)
                }"
            )
            if (file.isFile && file.name.substring(file.name.lastIndexOf('.')) in expectedExtensions) {
                voice =
                    _uiState.value.voice.copy(selectedFile = context.contentResolver.persistedUriPermissions[0].uri.let {
                        //DocumentsContract.buildDocumentUriUsingTree(it, DocumentsContract.get(it.authority, it.lastPathSegment, "audio/*"))
                        val cachedFile = File("${context.cacheDir}/${file.name}")
                        cachedFile.delete()
...
//                _uiState.value.advice.value = generatedResponse.text?.trim()
        ...
        try {
            ...
            val loadableList = if (_uiState.value.voice.response?.feedbackOfFeedback != null) {
                listOf(
                    ...
                    content("assistant") {
                        text("feedback: ${_uiState.value.voice.response?.transcription}")
                    }, content {
                        text("practical voice: ")
                    },
                    content {
            ...
            val model = GenerativeModel(
                ...
                generationConfig = generationConfig {
                    ...
                    responseSchema = Schema(
                        "resolved_feedback_and_transcription",
                        "The resolved feedback and transcription of the audio file.",
                        properties = mapOf(
                            "feedback" to Schema.str("Feedback", "Feedback of the voice."),
            ...
            val generatedResponse =
                chat.sendMessage("Give my voice a too brief feedback with desired emojis and the highest accuracy.")
            voice = (fromJson<VoiceResponse>(generatedResponse.text) ?: return).let {
                _uiState.value.voice.copy(response = VoiceResponse(it.feedback, it.transcription))
            }
    ...
    private fun uploadVoiceToCloud() {
        ApiClient.saltechAi.uploadVoice(
            BaseApplication.Ai.Gemini.apiKeys[0],
            _uiState.value.voice.selectedFile!!.asRequestBody("audio/*".toMediaType()).let {
