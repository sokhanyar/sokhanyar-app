...
private fun Launcher(
			...
			MainPage(
				...
				menuPageItems = listOf(
					...
					MenuPageItem(
						...
						disabledReason = "هنوز کمی دیگه کار داره .. 👨🏻‍🔧🧰\nمنتظر باشین! 😃😉"
					),
					MenuPageItem(
						iconResId = R.drawable.chat,
						title = "فضای گفتگو",
						onClick = {
							// TODO: Loads previous messages from database
							onPageWanted(BaseApplication.Page.GroupChatRoom)
						},
						comingSoon = true
					),
				)
	...
	AnimatedVisibility(
		uiState.activePages.last() == BaseApplication.Page.AiChatRoom,
		enter = fadeIn(),
		exit = fadeOut()
	) {
	...
	AnimatedVisibility(
		uiState.activePages.last() == BaseApplication.Page.Menu,
		enter = fadeIn(),
		exit = fadeOut()
	) {
