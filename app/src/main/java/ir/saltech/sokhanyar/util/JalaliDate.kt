...
fun gregorianToJalali(gy: Int, gm: Int, gd: Int): IntArray {
	var gregorianDayOffsets: IntArray = intArrayOf(0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334)
	var gy2: Int = if (gm > 2) (gy + 1) else gy
	var days: Int =
		355666 + (365 * gy) + ((gy2 + 3) / 4).toInt() - ((gy2 + 99) / 100).toInt() + ((gy2 + 399) / 400).toInt() + gd + gregorianDayOffsets[gm - 1]
...
fun jalaliToGregorian(jy: Int, jm: Int, jd: Int): IntArray {
	...
	var gd: Int = days + 1
	var daysInMonth: IntArray = intArrayOf(
		0,
		31,
	...
	var gm = 0
	while (gm < 13 && gd > daysInMonth[gm]) gd -= daysInMonth[gm++]
